...
namespace JiebaNet.Segmenter.Tests
{
    ...
    public class TestSegmenter
    {
        ...
        [TestCase]
        [Category("DateTime")]
        public void TestChineseYearOnly()
        {
            var seg = new JiebaSegmenter();
            
            // 测试"二零一零年"应该作为整体
            var result1 = seg.Cut("我是二零一零年出生的").ToList();
            Assert.That(result1, Contains.Item("二零一零年"), "二零一零年应该作为整体");
            
            // 测试"二〇一〇年"应该作为整体
            var result2 = seg.Cut("我是二〇一〇年出生的").ToList();
            Assert.That(result2, Contains.Item("二〇一〇年"), "二〇一〇年应该作为整体");
            
            // 测试阿拉伯数字年份
            var result3 = seg.Cut("我是2010年出生的").ToList();
            Assert.That(result3, Contains.Item("2010年"), "2010年应该作为整体");
            
            Console.WriteLine("测试1: " + string.Join("/", result1));
            Console.WriteLine("测试2: " + string.Join("/", result2));
        ...
        [TestCase]
        [Category("DateTime")]
        public void TestChineseDateTimeWithYear()
        {
            var seg = new JiebaSegmenter();
            
            // 测试"二零一零年五月"应该作为整体
            var result1 = seg.Cut("我是二零一零年五月出生的").ToList();
            Assert.That(result1, Contains.Item("二零一零年五月"), "二零一零年五月应该作为整体");
            
            // 测试"二〇一〇年五月"应该作为整体
            var result2 = seg.Cut("我是二〇一〇年五月出生的").ToList();
            Assert.That(result2, Contains.Item("二〇一〇年五月"), "二〇一〇年五月应该作为整体");
            
            // 测试阿拉伯数字年月
            var result3 = seg.Cut("我是2010年5月出生的").ToList();
            Assert.That(result3, Contains.Item("2010年5月"), "2010年5月应该作为整体");
        ...
        [TestCase]
        [Category("DateTime")]
        public void TestChineseFullDate()
        {
            var seg = new JiebaSegmenter();
            
            // 测试"二零一零年五月一日"应该作为整体
            var result1 = seg.Cut("我是二零一零年五月一日出生的").ToList();
            Assert.That(result1, Contains.Item("二零一零年五月一日"), "二零一零年五月一日应该作为整体");
            
            // 测试"二〇一〇年五月一日"应该作为整体
            var result2 = seg.Cut("我是二〇一〇年五月一日出生的").ToList();
            Assert.That(result2, Contains.Item("二〇一〇年五月一日"), "二〇一〇年五月一日应该作为整体");
            
            // 测试阿拉伯数字完整日期
            var result3 = seg.Cut("我是2010年5月1日出生的").ToList();
            Assert.That(result3, Contains.Item("2010年5月1日"), "2010年5月1日应该作为整体");
