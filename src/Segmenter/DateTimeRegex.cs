...
namespace JiebaNet.Segmenter
{
    ...
    public class RegexTimeRecognizer : ITimeRecognizer
    {
        ...
        // ========== 6. 时间（关键修复：支持"4:50"格式和中文时间整体识别） ==========
        // 格式1：HH:MM 或 HH:MM:SS（如 4:50, 14:30:00）
        // 格式2：中文时间表达（如 上午9点, 下午3点半, 晚上8点30分, 凌晨2点）
        // 注意：不使用\b，因为\b对中文无效
        private static readonly Regex TimeRegex = new(
            @"(?:^|(?<!\d))(?<hour>\d{1,2}):(?<minute>\d{2})(?::(?<second>\d{2}))?(?!\d)|" +
            @"(?<ampm>上午|下午|早上|晚上|凌晨|[" + C_后 + @"]午|傍晚|黄昏)?\s*" +
            @"(?<hour>\d{1,2}|[一二三四五六七八九十]{1,3})[" + C_点 + C_时 + @"]" +
            @"(?:\s*(?<minute>\d{1,2}|半|一刻|三刻)[分]?)?(?:\s*(?<second>\d{1,2})秒)?",
            RegexOptions.Compiled);
